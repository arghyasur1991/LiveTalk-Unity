...
public class CharacterManager : MonoBehaviour
{
    void Start()
    {
        ...
        string[] characterIds = LiveTalkAPI.Instance.GetAvailableCharacterIds();
        Debug.Log($"Found {characterIds.Length} characters");
        
        foreach (string id in characterIds)
        {
            // Check character format
            string format = LiveTalkAPI.Instance.GetCharacterFormat(id);
            bool isBundle = LiveTalkAPI.Instance.IsCharacterBundle(id);
            string path = LiveTalkAPI.Instance.GetCharacterPath(id);
            
            Debug.Log($"Character {id}: {format} format at {path}");
        }
        
        // Check recommended format for current platform
        bool shouldUseBundle = LiveTalkAPI.ShouldUseBundle(); // true on macOS
        Debug.Log($"Recommended format: {(shouldUseBundle ? "bundle" : "folder")}");
    }
}
```
