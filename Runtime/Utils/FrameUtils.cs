...
#region Conversion
        ...
        /// <param name="inputMin">Minimum value of input data range (default -1.0f)</param>
        /// <param name="inputMax">Maximum value of input data range (default 1.0f)</param>
        /// <returns>Frame</returns>
        public static unsafe Frame TensorToFrame(Tensor<float> tensor, float inputMin = -1f, float inputMax = 1f)
        {
            if (tensor == null)
            ...
            // Pre-calculate normalization parameters for performance
            float inputRange = inputMax - inputMin;
            bool needsNormalization = inputRange != 1f || inputMin != 0f;
            ...
            fixed (byte* pixelPtr = pixelData)
            {
                ...
                Parallel.For(0, height, y =>
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        // Apply normalization based on input range
                        if (needsNormalization)
                        {
                            // Normalize from [inputMin, inputMax] to [0, 1]
                            r = Mathf.Clamp01((r - inputMin) / inputRange);
                            g = Mathf.Clamp01((g - inputMin) / inputRange);
                            b = Mathf.Clamp01((b - inputMin) / inputRange);
                        }
                        else
                        {
                            // Input is already in [0, 1] range, just clamp
                            r = Mathf.Clamp01(r);
                            g = Mathf.Clamp01(g);
                            b = Mathf.Clamp01(b);
                        }
...
#region Transformations
        ...
        /// <summary>
        /// Resize frame to exact target dimensions
        /// </summary>
        ...
        /// <summary>
        /// Affine transform frame to exact target dimensions
...
#region Image Processing
        ...
        /// </summary>
        /// <param name="frame">Frame to dilate</param>
        /// <param name="kernelSize">Kernel size (odd number)</param>
        /// <returns>Dilated frame</returns>
        ...
        /// </summary>
        /// <param name="frame">Frame to erode</param>
        /// <param name="kernelSize">Kernel size (odd number)</param>
        /// <returns>Eroded frame</returns>
        ...
        public static unsafe Frame ApplySimpleGaussianBlur(Frame frame, int kernelSize)
        {
            if (frame.data == null)
            {
                Logger.LogError("[FrameUtils] Null frame passed to Gaussian blur");
                return frame;
            }
            
            if (frame.width <= 0 || frame.height <= 0)
            {
                Logger.LogError($"[FrameUtils] Invalid dimensions: {frame.width}x{frame.height}");
                return frame;
            }
            ...
            byte* tempPtr = (byte*)UnsafeUtility.Malloc(totalPixels * 3, 4, Unity.Collections.Allocator.TempJob);
            ...
            finally
            {
                ...
                UnsafeUtility.Free(tempPtr, Unity.Collections.Allocator.TempJob);
            }
            return outputFrame;
        }
