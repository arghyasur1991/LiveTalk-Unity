...
namespace LiveTalk.API
{
    ...
    public class CharacterPlayer : MonoBehaviour
    {
        ...
        private void ProcessNextSpeech()
        {
            ...
            _speechCoroutine = StartCoroutine(ProcessSpeechWithTransition(request.Text, request.ExpressionIndex));
        }

        private IEnumerator ProcessSpeechWithTransition(string text, int expressionIndex)
        {
            _state = PlaybackState.Speaking;
            StopIdleAnimation();
            OnSpeechStarted?.Invoke();
            
            Debug.Log($"[CharacterPlayer] Starting speech: {text.Substring(0, Math.Min(50, text.Length))}...");
            
            // 1. Generate speech audio + animation
            ...
            IEnumerator speechCoroutine = _character.SpeakAsync(
                ...
                onAudioReady: (stream, clip) =>
                {
                    ...
                    audioClip = clip;
                    audioReady = true;
                    Debug.Log($"[CharacterPlayer] Audio ready for speech");
                },
            ...
            // 2. Wait for audio to be ready
            yield return new WaitUntil(() => audioReady);
            
            if (audioClip == null || frameStream == null)
            {
                Debug.LogError("[CharacterPlayer] Audio or frame stream is null");
                _state = PlaybackState.Idle;
                _speechCoroutine = null;
                StartIdleAnimation();
                ProcessNextSpeech();
            ...
            // 3. Collect all speech frames
            List<Texture2D> speechFrames = new List<Texture2D>();
            while (frameStream.HasMoreFrames)
            {
                ...
                {
                    speechFrames.Add(awaiter.Texture);
                }
            }
            
            Debug.Log($"[CharacterPlayer] Collected {speechFrames.Count} speech frames");
            
            // 4. Append transition frames to return to idle frame 0
            var transitionFrames = GetTransitionToIdleFrames();
            speechFrames.AddRange(transitionFrames);
            
            // 5. Play frames synchronized with audio
            yield return PlayFramesSynchronized(speechFrames, audioClip);
            ...
            // 7. Start idle animation
            StartIdleAnimation();
            
            // 8. Process next speech in queue
            if (_speechQueue.Count > 0)
            {
                ProcessNextSpeech();
            }
