...
### Added
- **`DrivingMotionOptions.EyeExpressionGain`** (default 1): the expression gain applied to LivePortrait's four eye keypoints instead of `ExpressionGain`. A blink already spans the eye's full range in the driver; amplifying it pushed the lids past closed (lash line below the eye, socket bulging). Avatars are built with 1.
- **`LiveTalkAPI.MeasureMotionAsync(framesDir, csvPath)`** — the analytical validator. Runs a folder of frames through the same landmark-track → fixed-face-crop → motion-extractor path a driving clip takes and writes one CSV row per frame (pose, extractor scale, translation, the 63-dim expression vector and the 203 tracked landmarks). `Tools~/driving_clips/compare_motion.py` compares a clip's CSV with the CSV of what was rendered from it: a head-size gate (the output's extractor-scale range may not exceed the driver's own expression-leakage range) and, per geometric feature (eye openness, lip openness, mouth width, mouth-corner drop, inner/outer brow height, pitch/yaw/roll), the correlation and amplitude ratio of the transfer. Every number below comes from it; contact sheets are no longer how transfer is judged.
- **`DrivingMotionOptions.ScaleTransfer`** (default 0): how much of the driving clip's extractor `scale` change reaches the render. 1 is upstream's relative mode.

### Fixed
- **Every face crop was rotated ~40°.** `FaceAnalysis.ParsePt2FromPtX` applied the 106-point eye/lip indices to the 203-point `LandmarkRunner` output (upstream `parse_pt2_from_pt_x` dispatches on the point count). The eye→lip axis came out ~40° off, `flag_do_rot` rotated every source crop, every driving-frame crop and every per-frame landmark re-track by that, and the rotated bounding box made the face ~14 % smaller in the 256×256 extractor input. Consequences, all now gone: expressions read weak (the 0.35–0.75× transfer measured below was this), eyes overshot into skin blobs on blinks, mouth and eye motion was applied along a rotated axis so features drifted sideways, and the extractor's `scale` swung with expressions. Found by running the stock pipeline outside Unity on the same inputs and comparing the frame-0 extractor read (C# roll −40.6° on a level driver vs −0.2° in Python; after the fix −0.23° vs −0.18°, expression-delta correlation 0.79 → 0.98, output pixel MAD against the reference at a blink 6.5 → 1.8). `MotionPipelineVersion` 5 (6 with the eye gain below): every avatar rebuilds once.
- **Expression gain is 1.4** (was 2.0, tuned against the rotated crop) and **does not apply to the eyes** (`EyeExpressionGain` 1). On upright crops 1.0 already reads, 1.4 is clearly expressive, 1.8 starts to look pushed on the surprised mouth.
- **Removed `Resources/00000000.png`**, an unreferenced 853 KB sample image that shipped in every consumer build.
- Head size on upright crops: the driver's own extractor-scale range is 1.5 % (surprised) and the rendered output stays within 0.7 % with **or without** `ScaleTransfer = 0`; the 6 % / 15 % swell described below was the rotated crop. `ScaleTransfer = 0` remains the default as a harmless pin for static-camera clips.
- *(superseded above — the swell was the rotated crop)* **Head no longer swells during an expression.** The motion extractor's `scale` output is not a clean camera-distance signal: on a clip shot with a fixed camera it still rises 6 % through a surprise (jaw drop and raised brows read as a bigger head) and relative mode passed that straight onto the source — measured 15 % scale range on the rendered surprise against 6 % on its driver, i.e. the head grew ~11 % over the onset. Face-cropping the driving frames (kept: the extractor was trained on tight crops and expressions now arrive at the right size) did not fix it, and the ±8 % clamp merely hid it — both were previously reported as a fix on the strength of a motion-mask proxy that could not see it. With `ScaleTransfer = 0` the render keeps the source's head size for the whole clip: 4.8 % on the same clip, within the driver's own range. `MotionPipelineVersion` 4.
- *(superseded above — these numbers were measured through the rotated crop)* **Expression gain is 2.0 for every bundled clip** (was 1 / 1.35 / 1.7 per expression). Measured at gain 1 LivePortrait passes 0.35–0.75 of the driver's eye, brow and lip change onto a different identity while pose passes at ~1.0; at 2.0 those land at 0.75–1.1 with no tearing, at 2.5 the brows overshoot (1.25×) and eye correlation drops. `ExpressionGain` is applied in `EditMotion`, so the raw (`motion == null`) path does not see it — pass options to measure the shipped pipeline.
- Driving frames are face-cropped before motion extraction (fixed crop from frame 0, same 2.3 / −0.125 framing as the source). Per-frame cost of the crop and of the extractor is now logged per clip (`face crop N ms, motion extractor N ms`); it runs at avatar-build time only, nothing changes at playback.
