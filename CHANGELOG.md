...
- **`avatar.json` records `fps`, `loopable` and `motionPipelineVersion`.** `Avatar.FrameRate` / `Avatar.IsLoopable` (and the matching `Character` idle fields) expose them. Legacy folders read as 25 fps, not loopable.
...
- Driving frames are face-cropped before motion extraction (fixed crop from frame 0, same framing as the source).
...
- **Avatar ids include a fingerprint of the bundled clips** (`Avatar.DrivingClipsHash`) and the driving-frame recipe (`motionPipelineVersion`). Replacing a clip or changing the recipe rebuilds every avatar.
