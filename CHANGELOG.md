...
The face crop LivePortrait works from was rotated ~40° (see Fixed). That was the cause of weak expression transfer, sideways lip/eye drift, blink overshoot, and the extractor's scale swinging with the jaw. The seven bundled driving clips are re-rendered at 25 fps, rest-to-rest. Keypoint-space motion edits from 2.1.0 (`DrivingMotionOptions`, resample, loop blend, expression gain, scale pin) are **removed** — they were compensating for the rotated crop; the clips already loop and share the lip-sync clock. `MotionPipelineVersion` is 8, so **every existing avatar folder is rebuilt once** on its next `CreateAvatarAsync`.
